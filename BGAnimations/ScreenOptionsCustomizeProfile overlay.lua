...
local dir = PROFILEMAN:LocalProfileIDToDir(GAMESTATE:GetEditLocalProfileID())
--Set up vars to edit
ActiveModifiers["EDIT"] = table.shallowcopy(PlayerDefaults)
LoadProfileCustom(profile,dir, true)
...
local function item_value_to_text(item, value)
	...
	elseif item.item_type == "list" or item.item_type == "customList" then
...
local profileIconList = {};
for i,icon in ipairs(FILEMAN:GetDirListing(THEME:GetPathG("","Avatars").."/")) do
	profileIconList[#profileIconList+1] = {
		setting=icon, display_name=icon:sub(1, -5)
	}
end;
...
local function exit_screen()
	...
	SaveProfileCustom(profile,dir, true)
...
local function input(event)
	...
	if cursor_on_menu == "main" then
		if button == "Start" then
			...
			elseif item.item_type == "list" then
				...
				update_list_cursor()
			elseif item.item_type == "customList" then
				cursor_on_menu= "customList"
				active_list= menu_items[menu_pos].list
				list_pos= calc_list_pos(ActiveModifiers["EDIT"][menu_items[menu_pos].get], active_list)
				update_list_cursor()
	...
	elseif cursor_on_menu == "customList" then
		if button == "MenuLeft" or button == "MenuUp" then
			if list_pos > 1 then list_pos= list_pos - 1 end
			update_list_cursor()
			menu_values[menu_pos]:playcommand("PressLeft")
		...
		elseif button == "Start" or button == "Back" then
			if button ~= "Back" then
				ActiveModifiers["EDIT"][menu_items[menu_pos].set] = active_list[list_pos].setting
			end
			local valactor= menu_values[menu_pos]
			left_showing= false
			right_showing= false
...
for i, item in ipairs(menu_items) do
	local item_y= menu_start + ((i-1) * menu_y_spacing)
	-- 3.  The Menu Items
	...
	if item.get then
		local value_text;
		if item.item_type == "customList" then
			value_text = item_value_to_text(item, ActiveModifiers["EDIT"][item.get])
			--value_text = "Test";
		else
			value_text= item_value_to_text(item, profile[item.get](profile))	
		end;
		-- 4.  The Menu Values
		...
		local value_args= {
			...
			InitCommand= function(self)
				...
				menu_values[i]= self
				self:xy(value_x, menu_start + ((i-1) * menu_y_spacing))
			end,
			Def.BitmapText{
...
args[#args+1]= Def.Quad{
	Name= "fader", InitCommand= function(self)
		fader= self
		self:setsize(270, #menu_items * menu_y_spacing)
		self:horizalign(left)
		self:vertalign(top)
...
args[#args+1] = LoadFont("Common Normal")..{
		Text="Editing profile \'"..profile:GetDisplayName().."\'";
